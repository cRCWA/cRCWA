    ...
    if(strcmp(otype,"n")==0) {
        ...
        if (crtr == &NormalField) {
        	...
        	if (NormalFieldSym.Nxx_input.getNrow() > 0 and 
        		...
        		double px, py;
        		int posx, posy ;
				for (int l=0; l<out_x.getNcol(); ++l) {
				    for(int m=0; m<out_x.getNrow(); ++m) {
				        px=(double(l)*p->tot_x)/out_x.getNcol()-p->tot_x/2.0;
				        py=(double(m)*p->tot_y)/out_x.getNrow()-p->tot_y/2.0;
				        posx = round((px/p->tot_x+0.5)*(NormalFieldSym.Nxx_input.getNcol()-1));
        				posy = round((py/p->tot_y+0.5)*(NormalFieldSym.Nxx_input.getNrow()-1));
				        
				        out_x(m,l)=sqrt(NormalFieldSym.Nxx_input(posy, posx));
				        out_y(m,l)=sqrt(NormalFieldSym.Nyy_input(posy, posx));
				    }
				}
			} else {
			    throw parsefile_commandError("inpnf: programing error :'matdev nfs'"
    ...
    )
{
    ...
    for(ii=0;ii<Ny;++ii) {
        for(jj=0;jj<Nx;++jj) {
            ...
            // In the original article from Lalane "Perfectly matched layers as
            // nonlinear coordinate transforms: a generalized formalization"
            // There is a (-1)^k term that account for the shift of the
            // pattern used to change the derivative at +/-d/2, that is 
            // exp(i*pi*k)+exp(-i*pi*k) = 2*cos(pi*k) =2*(-1)^k. In crcwa, 
            // the structure is computed fromx=0 to x=d (and not from x=-d/2 to 
            // x=d/2 in Lalane's article) the shift of pattern is at 0 or d, 
            // that leads to a multiplication by exp(i*2*pi*k)=1.
...
complex<double> section::expectedRefractiveIndex(double px, double py)
{
    ...
    if(background.getNrow()>0) {
        ...
        int posx = round((px/father->tot_x+0.5)*(background.getNcol()-1));
        int posy = round((py/father->tot_y+0.5)*(background.getNrow()-1));
