...
\chapter{The \textsf{draw} utility}
\label{ch_draw}
\section{Introduction}
\textsf{draw} utility can be used either as a standalone tool or as part of \afmm command to draw structure and generate normal field. Standalone version is user-fiendly and can be used to generate the command for \afmm. Integrated command in \afmm is equivalent as :
\begin{itemize}
\item 1- generate index and normal filed with \textsf{draw} standalone
\item 2- input refractinve index with \textsf{indfile}
\item 3- input normal field with \textsf{matdev nf} or\textsf{matdev nfs}
\end {itemize}
\section{Help}
Help can be easily obtained with :
\begin{lstlisting}
$ ./draw --help 
...
To generate a rectangle waveguide of 200x500nm² with refractive index of 1.5 in a computation window of 1x1µm², we can use the interactive tool:

\begin{lstlisting}
$ ./draw
Draw Utility part of cRCWA sofware suite. 
Use ./draw --help for usage options or answer the following questions:
 
filename? structure.rid
size of computation window in x? 1e-6
size of computation window in y? 1e-6
number of layers? 1
shape type (c, e, r or p)? r
Internal real index? 1.5
Internal imaginary index? 0
width of the rectangle along x (between 0 and 1e-06)? 200e-9
width of the rectangle along y (between 0 and 1e-06)? 500e-9
Origine X (between -5e-07 and 5e-07)? 0
Origine Y (between -5e-07 and 5e-07)? 0
External real index? 1
External imaginary index? 0
Command executed: draw [structure.rid 1e-06 1e-06] 1 r 1.5 0 2e-07 5e-07 0 0 1 0 
Refractive index map generated
Normal field maps generated
Writing in structure.rid
writing normal field files: structure.rid_nvf_x and structure.rid_nvf_y
\end{lstlisting}

\textsf{draw} utility has indicated the command parameters to use both as \afmm  command or in the stand alone \textsf{draw} utility. we can re-run the generation with:
\begin{lstlisting}
$ ./draw [structure.rid 1e-06 1e-06] 1 r 1.5 0 2e-07 5e-07 0 0 1 0 
\end{lstlisting}

\subsection{Circle}
To generate a circular waveguide of radius 300nm with refractive index of 1.5 in a computation window of 1x1µm², we can use the interactive tool:
...
size of computation window in y? 1e-6
number of layers? 1
shape type (c, e, r or p)? c
Internal real index? 1.5
Internal imaginary index? 0
radius along x (between 0 and 5e-07)? 300e-9
Number of edges? 50
Origine X (between -5e-07 and 5e-07)? 0
Origine Y (between -5e-07 and 5e-07)? 0
Shift angle (deg)? 0
External real index? 1
External imaginary index? 0
Command executed: draw [structure.rid 1e-06 1e-06] 1 c 1.5 0 3e-07 50 0 0 0 1 0 
Refractive index map generated
Normal field maps generated
Writing in structure.rid
writing normal field files: structure.rid_nvf_x and structure.rid_nvf_y
...
\textsf{draw} utility has indicated the command parameters to use both as \afmm  command or in the stand alone \textsf{draw} utility. we can re-run the generation with:
\begin{lstlisting}
$ ./draw [structure.rid 1e-06 1e-06] 1 c 1.5 0 3e-07 50 0 0 0 1 0
\end{lstlisting}

\subsection{Plotting results}
The \texttt{o2g} is used to plot the output of \texttt{draw}:

\begin{lstlisting}
$ ./o2g structure.rid structure.ind
$./o2g structure.rid_nvf_x structure.ind_nvf_x
$ ./o2g structure.rid_nvf_y structure.ind_nvf_y
$ paste structure.ind_nvf_x structure.ind_nvf_y > structure.ind_nvf
\end{lstlisting}

With gnuplot, we can run the following commands:
\begin{lstlisting}
gnuplot> set pm3d map
gnuplot> splot 'structure.ind' u 1:2:3
gnuplot> plot 'structure.ind_nvf' every 5:5 u ($1*1e6):($2*1e6):($3/50):($7/50) with vectors
\end{lstlisting}
...
Adding \lstinline!material! command to import refractive index files.
Adding \lstinline!sprintf! command to create string variables for custom filenames.
Renaming carpet to \lstinline!ensureconv! for more explicit name. \lstinline!ensureconv! has a parameter to activate or desactivate the function. \lstinline!ensureconv! is now activated by default
Adding \lstinline!inpstruct3d! to export the 3D structure to better display structure with z-variations. 

\section{Version 1.5.3}
Updating compilation procedure. Enable compilation under Winndows.

\section{Version 1.5.2}
Adding \lstinline!draw! command and \textsf{draw} utility for drawing structure and automatically generating normal field for improve convergence.
\lstinline!inpnf! has been introduced to export the normalfield generated by the \lstinline!draw! command.
\lstinline!...! has also been introduce to cut on instruction line into multiple lines. While issuing an error, the line numer is given to ease debugging.

\section{Version 1.5.1}
The command \lstinline!monitor! has been debugged for non-centered case and automated tests where added. \lstinline!outdata sz! was also added to extract the z-component of the Poynting vector.
