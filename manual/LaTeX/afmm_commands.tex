...
For systems where the GNU make utility can be run, a makefile is available. 
This file should be modified in order to specify where to find the LAPACK, BLAS, libf2c as well as FFTW3 libraries.\footnote{Probably, the easiest way to use those libraries is to install them separately. In Unix systems, a static library file normally has the \lstinline!.a! extension and a shared library has the \lstinline!.so! extension.} 
...
\textsf{Afmm} can be used in three ways. \marginpars{You must know this.} If a file is specified in the command line, \afmm\ reads it and processes each line. If not, \afmm\ can be used interactively from the command line. 
...
The third way is to use \afmm\ as a Python module, as shown in appendix \ref{ch_python}.
...
This command specifies the number of harmonics to be considered in the Fourier decomposition of the fields.
Usage: \lstinline!harmonics Nx Nx!
    
Parameters:
\begin{itemize}
\item \lstinline!Nx!: number of harmonics on the $x$ axis (or $r$ axis if the structure is bent).
\item \lstinline!Ny!: number of harmonics on the $y$ axis (or $z$ axis if the structure is bent).
\end{itemize}
Examples:
\begin{lstlisting}
harmonics 43 43
\end{lstlisting}
Use during calculations 43 harmonics in each direction, which means the continuous term as well as $\pm 42$ Fourier terms. The memory occupation needed for the calculations is $O(S_xS_y \times S_xS_y)$. There are no restrictions to the number of harmonics but the finite memory of the computer or workstation which is being used. 
...
we obtain that:
\begin{itemize}
\item The permeability and permittivity will be represented with $S_x=21$ coefficients in $x$ (DC+fundamental+harmonics up to order 10) and 1 coefficient in $y$ (DC only).
\item The resulting calculated electric and magnetic fields will contain 11 coefficients in $x$ (DC+fundamental+harmonics up to order 5) and 1 coefficient in $y$ (DC only).
\end{itemize}
...
\subsection{MATERIAL: Load refractive index files and interpolate to the wavelength}
\label{subsec_material}
Usage: \lstinline!material mult filename tag!

Parameters:
\begin{itemize}
\item \lstinline!mult!: a multiplier to convert the wavelength from file in meter.
For example, if the wavelength in the input file is specified in micrometers, mult 
should be 1e-6.
\item \lstinline!filename!: name of the file to be used (it should not contain spaces).
\item \lstinline!tag!: name of the material that can then used instead of nr and ni 
    	parameters in \lstinline!rectangle!, \lstinline!draw!, \lstinline!substrate!
    	or \lstinline!print! commands...
\end{itemize}

Notes:
\begin{itemize}
\item The file format to be used will be as follows. The first line is ignored.
...
\item \lstinline!tag! is actually simply an array with real and imaginary part of
the refractive index. \lstinline!tag(0)! return the real part of the refractive
index
\end{itemize}


\subsection{INPSTRUCT3D: Show the 3d input structure}
\label{subsec_inpstruct3d}
Write on a file the permittivity or permeability chart of the input structure (stands for INPut STRUCTure 3D). This may be useful to check if it has been 
...
is represented, given the number of harmonics taken into account, unless the \lstinline!im! type of representation is chosen. In this case, the represented structure is the one originally specified, without passing by the Fourier transforms.
    ...
    Usage: \lstinline!inpstruct3d type size_x size_y file!
    
    
    Parameters:
    \begin{itemize}
    \item
    \lstinline!type!    must be \lstinline!{i|im|ex|ey|ez|mux|muy|muz}!, \lstinline!i! indicates the 
            refractive index
            ...
            permeability tensor. \lstinline!im! indicates the original refractive index distribution, without having passed by a Fourier transform.
    \item
...
\subsection{CARPET: Depreciated see ENSURECONV}
\label{subsec_carpet}
This command is depreciated for \afmm\ v. 1.5.3 and later and replaced by \lstinline!ensureconv!

    Usage: \lstinline!carpet!
    
\subsection{ENSURECONV: ensure proper propagation modes}
\label{subsec_ensureconv}
The \lstinline!ensureconv! command forces all modes to have a negative imaginary part for the effective index. It is useful when calculating the propagation of the fields, since a positive imaginary part would result in a field which can quickly reach very high value when propagating. This often result in a convergence issue. It should be noted that what it is done by the \lstinline!ensureconv! command is somewhat artificial and a rigorous mathematical justification is still missing\cite{hench2008rcwa}. It just seems to work.
This command should be used before \lstinline!solve!.

    Usage: \lstinline!ensureconv t/f!
    
    Parameter:
    \begin{itemize}
    \item\lstinline!t/f!: \lstinline!t! to activate ensure convergence, \lstinline!f! to desactivate. 
    \end{itemize}
...
\subsection{OUTDATA: Calculate additional output data during propagation}
 This command sets up for the calculation of additional output data during the propgation of the fields. This command should be used before the \lstinline!propagation! command.
    ...
    \begin{itemize}
		...
		\item if \lstinline!t=sz!, it computes the z-component of the Poynting vector 
		(Power flowing in the z direction) 

		Usage: \lstinline!outdata sz rimc filename! 
		 
		where: 
		\begin{itemize}
	    	\item\lstinline!rimc!    must be \lstinline!{r|i|m|c}! and indicates if  the real part should be written
		    (\lstinline!r!), the imaginary part (\lstinline!i!), the magnitude (\lstinline!m!) or the complex 
		    value (\lstinline!c!) of the Poynting vector.
			\item \lstinline!filename! is the name of the file on which Poynting vector will be saved.
    	\end{itemize}
...
\subsection{DRAW: draw the structure}
\label{subsec_draw}

Generates a structure by drawing polygones. When 2 polygones overlaps, the first defined polygone is drawn on top of the others.

Usage: \lstinline!draw [...] nb_shapes shape_type shape_parameters ... n_ext k_ext OPTIONAL_PARAMETERS!


Parameters:
\begin{itemize}
\item \lstinline!nb_shapes!: number of shapes to be drawn. \lstinline!shape_type shape_parameters! are input for each shape.
\item \lstinline!shape_type!: defines the type of shape can be \lstinline!{c|e|r|p}!.
    \begin{itemize}
    	\item if \lstinline!shape_type=c!, the polygone is generated with points on a circle.
        
        	\lstinline!shape_parameters! = \lstinline!nr_shape ni_shape rx nb_edges px py shift_angle!
        	
        	where:
		    \begin{itemize}
	    		\item \lstinline!nr_shape!: shape refractive index, real part
				\item \lstinline!ni_shape!: shape refractive index, imaginary part
				\item \lstinline!rx!: radius of the circle used to compute polygone points
				\item \lstinline!nb_edges!: number of edged of the polygone
				\item \lstinline!px!: $x$ position of the centre of the shape 
				\item \lstinline!py!: $y$ position of the centre of the shape
				\item \lstinline!shift_angle!: angle of rotation of the shape. For \lstinline!shift_angle=0!, the side of the polygone is perpendicular to the $x$ axis.		
	    	\end{itemize}

    	\item if \lstinline!shape_type=e!, the polygone is generated with points on an elipse.
        
        	\lstinline!shape_parameters! = \lstinline!nr_shape ni_shape rx ry nb_edges px py shift_angle!
        	
        	where:
		    \begin{itemize}
	    		\item \lstinline!nr_shape!: shape refractive index, real part
				\item \lstinline!ni_shape!: shape refractive index, imaginary part
				\item \lstinline!rx!: radius in $x$ direction of the elipse used to compute polygone points
				\item \lstinline!rx!: radius in $y$ direction of the elipse used to compute polygone points
				\item \lstinline!nb_edges!: number of edged of the polygone
				\item \lstinline!px!: $x$ position of the centre of the shape 
				\item \lstinline!py!: $y$ position of the centre of the shape
				\item \lstinline!shift_angle!: angle of rotation of the shape. For \lstinline!shift_angle=0!, the side of the polygone is perpendicular to the $x$ axis.		
    	...
    	\item if \lstinline!shape_type=r!, rectangle is generated.
        
        	\lstinline!shape_parameters! = \lstinline!nr_shape ni_shape wx wy px py!
        	
        	where:
		    \begin{itemize}
	    		\item \lstinline!nr_shape!: shape refractive index, real part
				\item \lstinline!ni_shape!: shape refractive index, imaginary part
				\item \lstinline!wx!: rectangle $x$ width
				\item \lstinline!wy!: rectangle $y$ width
				\item \lstinline!px!: $x$ position of the centre of the shape 
				\item \lstinline!py!: $y$ position of the centre of the shape
	    	\end{itemize}
	    	
    	\item if \lstinline!shape_type=p!, the point coodinate of the polygone are explicitly defined
        
        	\lstinline!shape_parameters! = \lstinline!nr_shape ni_shape nb_edges x0 y0 ... xN yN!
		    ...
		    \begin{itemize}
				...
				\item \lstinline!ni_shape!: shape refractive index, imaginary part
				\item \lstinline!nb_edges!: number of edged of the polygone
				\item \lstinline!x!: for each edge define the $x$ coordinate
				\item \lstinline!y!: for each edge define the $y$ coordinate
	    	\end{itemize}  	
	\end{itemize}
\item \lstinline!nr_sur!: surronding medium refractive index, real part
\item \lstinline!ni_sur!: surronding medium index, imaginary part
\item \lstinline!OPTIONAL_PARAMETERS!: optional parameters:
    \begin{itemize}
    	\item \lstinline!-o ox oy! set the preferential orientation of the normal field. When not set, the normal field is radial.
			\begin{itemize}
				\item \lstinline!ox!: $x$ component of the normal field
				\item \lstinline!oy!: $y$ component of the normal field
			\end{itemize}	
    	\item \lstinline!-c cx, cy! set the cordinate of the point where the normal field is pointing to.
    		For example, when the point is inside a polygone,
            normal field will be close to radial with a vortex inside the
            shape. When the point is pointing outside the shape, the 
            normal field parallel to each other pointing toward the specified point 
            cx and cy are the x- y- coordinate of the center point.
            When not set, it is centered on the polygone.

			\begin{itemize}
				\item \lstinline!cx!: $x$ coordinate of the center point
				\item \lstinline!cy!: $y$ coordinate of the center point
			\end{itemize}	
    	\item \lstinline!-no-nf! only generate the refractive index, not the  
             normal field. Previously computed normal field will be used instead.
...
Notes:
	This command usually results in more accurate results that using \lstinline!substrate! and \lstinline!rectangle! commands as it is automatically seting the normal vector field.
    This command should be used once the size of the calculation window (command \lstinline!size!)  and    the number of space harmonics to be taken in the calculations (command \lstinline!harmonics!)  have been defined.
    This command implicitly defines the maximum value of the effective index for a mode to be considered ``interesting'' when using the \lstinline!solve! command.

For example:
\begin{lstlisting}
draw 1 r 3.46 0.0 500e-9 200e-9 0 0 1 0
\end{lstlisting}
Define a $3.46$ refractive index (silicon) rectangular waveguide, $500\nm \times 200\nm$, centreed in the calculation window in air (refractive index of 1). The normal field is generated and used to improve convergence: better accuracy is reach for a smaller number of harmonics. This is equivalent to generate the normal field with external tool and run:
\begin{lstlisting}
substrate 1 0
rectangle 3.46 0.0 500e-9 200e-9 0 0
matdev nf 1e-6 nv_x.rid nv_y.rid
\end{lstlisting}

Example 2:
\begin{lstlisting}
draw 1 c 3.46 0 200e-9 50 0 0 0 1 0 
\end{lstlisting}
Define a $3.46$ refractive index (silicon) circular waveguide (50 points spread on a circle) with $200\nm$ radius, centreed in the calculation window, in air (refractive index of 1). The normal field is generated and used to improve convergence: better accuracy is reach for a smaller number of harmonics.


Example 3:
\begin{lstlisting}
draw 2 c 3.46 0 200e-9 8 0 0 0 p 1.44 0 4 400e-9 400e-9 ...
	-400e-9 400e-9 -400e-9 -400e-9 400e-9 400e-9 1 0 
\end{lstlisting}
Define a $3.46$ refractive index (silicon) hexagonal waveguide (8 points spread on a circle) with $200\nm$ radius, centreed in the calculation window; inside a square waveguide of $1.44$ of refractive index (SiO2), in the air. The normal field is generated and used to improve convergence: better accuracy is reach for a smaller number of harmonics.
...
\subsection{INPNF: Show the input normal-field}
\label{subsec_inpnf}
Write on a file the normal field considered in the computation (stands for INPut Normal-Field). This may be useful to check if it has been 
defined correctly by the input file, as well as to check how well the structure
is represented, given the number of harmonics taken into account, unless the \lstinline!nm! type of representation is chosen. In this case, the represented normal-field is the one originally specified, without passing by the Fourier transforms.
    
    Usage: \lstinline!inpnf type size_x size_y file_x file_y!
    
    \begin{itemize}
    \item
    \lstinline!type!    must be \lstinline!{n|nm|nmo}!, \lstinline!n! indicates the 
            normal field extracted. \lstinline!nm! indicates the original normal field distribution, without having passed by a Fourier transform. \lstinline!nmo! is the same as \lstinline!nm!, but the output is done in the Optiwave format.
    \item
    \lstinline!size_x!  number of points to be plot in the x direction
    \item
    \lstinline!size_y!  number of points to be plot in the y direction
    \item
    \lstinline!file_x!    name of the file which should be written the x component of the normal field
    \item
    \lstinline!file_y!    name of the file which should be written the y component of the normal field
    \end{itemize}
...
\begin{itemize}
	\item
	As the sign of the normal field is arbitrary when considering the normal field implementation, only the absolute of the normal field is stored in (when considering symmetry). The return normal field is thus the absolute value.
	\item For \lstinline!type=n!, the normal-field considered is obtained by an inverse Fourier transform.
	If the number of point given by \lstinline!size_x! and \lstinline!size_y! is bigger than 
	the corresponding size of the normal-field, a zero padding technique is applied 
	before calculating the inverse transform of the matrix obtained. This allows to
	represent a given distribution with a comfortable number of point even if the
	number of harmonics taken is much less. No magic: it is just that the results
	are interpolated.
...
\subsection{...: Continuing the command on the next line}
You can cut one instruction line into several line if you add \lstinline!...! at the end of the cut line. Note that all text after the \lstinline!...! are considered as comments.

For example:
\begin{lstlisting}
size 2e-6 ... All character after the ... are considered as comments
	200e-9 
\end{lstlisting}
...
\subsection{PRINT: Print a line at the output}
	...
	Note: space between elements are set by \lstinline!addspace! command
\subsection{SPRINT: Print a line to a string variable}
    This command make \afmm\ echo all the given parameters in a given variable. 
    This is very useful to create custom filenames for output files.
    
    Usage: \lstinline!sprint variable whatever you want!
    
    Parameter:
    \begin{itemize}
    \item\lstinline!variable! the name of the variable where to store the 
    printed text
    \end{itemize}
