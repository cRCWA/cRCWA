...
printf "Testing multi-propagation 2: "
$1 multiplepropagation2.fmm >test_output.txt
...
if awk -f awk_scr.awk test_results_x.txt test_reference_x.txt && awk -f awk_scr.awk test_results_z.txt test_reference_z.txt  &&  awk -f awk_scr.awk test_results_y.txt test_reference_y.txt
then
