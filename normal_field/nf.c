	...
	for (c=0; c<4; ++c) {
		for (k=0 ; k < nbp; ++k) {
			...
			if (i == m && j == n) {	
		    	...
		    	Nx[n][m] = Nx[n][m] + cx/abs((double)(pow(i-m,2) +
		    		pow(j-n,2)));                       
		    	Ny[n][m] = Ny[n][m] + cy/abs((double)(pow(i-m,2) + 
		    		pow(j-n,2)));     
			} 
		}
	}
...
int interpolateDisc (double Nx[NBPnf][NBPnf],double Ny[NBPnf][NBPnf], int centerx, int centery,int radiusx, int radiusy, int i1,int j1,int i2,int j2,int i3,int j3,int i4,int j4,int m,int n)
{
	...
	for (k=0 ; k < nbp; k++ )
	{
		...
		{
		    Nx[n][m] = Nx[n][m] + cos(alpha)/ abs((double)(pow(i-m,2) +
		    	pow(j-n,2)));                       
		    Ny[n][m] = Ny[n][m] + sin(alpha)/ abs((double)(pow(i-m,2) + 
		    	pow(j-n,2)));     
		} 
		/*Nx[j][i] = cos(alpha) ;
		Ny[j][i] = sin(alpha) ;*/
